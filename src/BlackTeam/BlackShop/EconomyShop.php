...
class EconomyShop extends PluginBase implements Listener{
	...
	public function onPlayerTouch(PlayerInteractEvent $event){
		...
		if(isset($this->shop[$loc])){
			...
			if(!$player->getInventory()->canAddItem(ItemFactory::get($shop["item"], $shop["meta"]))){
			...
			if($shop["price"] > $money){
				...
				$player->getInventory()->addItem(ItemFactory::get($shop["item"], $shop["meta"], $shop["amount"]));
